...
* 🛡️ **Security-First:** No hardcoded IPs, ports, or credentials. Everything is managed via encrypted `config.json` (ignored by Git).
...
* 🛡️ **Bảo mật tuyệt đối:** Không lưu cứng IP, Port hay mật khẩu trong code. Tất cả cấu hình nằm trong file `config.json` (đã được chặn bởi Git).
...
* 🛡️ **安全第一：** 代码中无任何硬编码的 IP、端口或凭据。所有敏感信息均通过 `config.json` 管理。
